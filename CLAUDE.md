...
- **Hooks — always verified + repaired, unconditional.** The full squadrant-owned hook set (`SessionStart` / `UserPromptSubmit` / `PreToolUse` incl. the `AskUserQuestion` tool matcher / `Stop` / `Notification` / `SessionEnd` / `PermissionRequest` / `StopFailure`) is checked on every run. A missing hook is repaired and a one-line warning is logged. The `AskUserQuestion` → `squadrant hooks claude ask-question` mapping is what makes crew-blocked signalling work (#560); a machine that never had it — or had it clobbered — used to lose blocked-signalling silently. It no longer does. `PermissionRequest` (#760) fires ~6s before the matching `Notification`, with `tool_name`/`tool_input` directly on the payload — a richer, earlier `task.blocked` source; `Notification` itself is classified by the structured `notification_type` field (not English substring matching) with the old substring test kept only as a fallback for clients that omit the field. `StopFailure` (#763) fires when a crew's turn dies on an API error (529/overload/etc) — previously invisible, surfacing only later as a misleading watchdog stall — and maps to `task.turn.failed`, never `task.failed` (anti-#2576: no hook may terminalize a task).
...
Squadrant is replacing screen-scraped liveness/delivery inference with native agent control APIs as ground truth. Lives in `@squadrant/core` (`src/captain-channel.ts`, `src/control-channel.ts`, `src/lifecycle-source.ts`), fed by the 5 `LifecycleSource` implementations (`CmuxStore`, `NativeHook`, `CodexAppServer`, `ClaudePeerRegistry`, `OpencodeControl` — #333) that already replaced the old title-sweep liveness model.

- **`controlChannel`** — per-agent-type setting (`off` / `shadow` / `on`). Both `claude` and `opencode` ship `on` by default: delivery verdicts for crew turns come from an agent receipt, not pane-scraping. `claude` was cut over 2026-08-20; `opencode` was live-proven 2026-09-19 (single-crew send + cross-worktree no-misroute) and is now on by default too. `off → shadow` needs a daemon bounce; `shadow → on` does not.
