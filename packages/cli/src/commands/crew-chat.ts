...
// DEPRECATED alias for `squadrant crew spawn --agent codex`. Kept so external
...
export const crewChatCommand = new Command("chat")
  .description("[DEPRECATED] alias for `squadrant crew spawn <project> '(interactive)' --agent codex`.")
  ...
  .action(async (opts: { provider: string; project: string; cwd: string; model?: string; approval: boolean }) => {
    process.stderr.write(
      "⚠️  `squadrant crew chat --provider codex` is deprecated. " +
      "Use: squadrant crew spawn <project> '(interactive)' --agent codex" +
