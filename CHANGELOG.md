...
- **Claude's subagent + small/fast model slots now stay in sync with the routed model, so they no longer 400 on a custom upstream (#855).** A router upstream (e.g. opencode-go) serves only the routed model, so `CLAUDE_CODE_SUBAGENT_MODEL` / `ANTHROPIC_SMALL_FAST_MODEL` left at an Anthropic alias made every subagent/small request fail with `400 … Model is unavailable`. `buildRouterEnv` now mirrors the resolved model into both slots for a routed spawn, and `installClaudeHooks` reconciles them against `ANTHROPIC_MODEL` in `~/.claude/settings.json` on every boot when the base URL is a custom (non-`api.anthropic.com`) upstream — filling absent keys, aligning bare Claude aliases / `claude-…` ids, and leaving custom ids untouched. `native` spawns stay byte-for-byte unchanged and the `defaults.claudeEnv` non-clobbering precedence is preserved.
- **`crew answer` can now answer an opencode permission prompt (#856).** `runCrewAnswer` → `parseModalOptions` recognised only claude's numbered `❯ N. Label` modal, so an opencode permission dialog — a left-bordered `△ Permission required` panel with a space-separated `Allow once   Allow always   Reject` footer and a `⇆ select  enter confirm` hint bar — parsed as `null` and the command refused with "no interactive option prompt visible", deadlocking any opencode crew (squadrant's default agent) on an `external_directory`/`bash`/`edit` prompt. A new `parseOpencodePermissionOptions` + a `parseModal` wrapper now report the option list *and* the arrow axis per shape; `runCrewAnswer` drives `Left`/`Right` + `Enter` for the opencode bar and keeps claude's `Up`/`Down` path byte-for-byte unchanged. cmux pane reads are plain text, so opencode's background-colour highlight is invisible — its known first-option default is treated as the current selection, with `--expect`/text matching intact.
- **A crew that stays `blocked` now re-notifies instead of going silent (#857).** A *different* question arriving while a task was already `blocked` hit `firePush`'s "same state" guard and was dropped; it now fires a fresh `CREW BLOCKED` (a literally-identical repeat stays suppressed, preserving [#174](https://github.com/tu11aa/squadrant/issues/174)). Separately, the sweep emits exactly one `CREW BLOCKED REMINDER` once a crew has been continuously blocked past a 30-minute threshold (explicitly the same question, not a new one) and then stays quiet for the rest of the episode; a later block re-nudges. Default threshold `DEFAULT_BLOCKED_RENUDGE_MS` (overridable via `DaemonDeps.blockedRenudgeMs`), skipped while the operator holds the crew.
- **Release-CI npm verification is now advisory, no longer fatal.** A registry replication lag past the publish could fail the job and skip the tag/GitHub Release (which stranded the `v0.22.0` release). The verify step now widens its window (18 → 36 attempts) and warns and continues on a timeout rather than exiting non-zero, so the tag and GitHub Release are always created.

### Documentation

- Auto-gate provider-agnostic design, implementation and classifier plans under `docs/specs/`; `defaults.gate.engine` schema, `crew answer` dialog shapes, and blocked re-notification in `docs/reference.md`; a config/override-compatibility note in `CONTRIBUTING.md`.
...
- **Two-stage delivery ACK, typing lifecycle, and an unresponsive-captain watchdog for Telegram (#838, #839, #840).** Stage 1 rides a 👍 `setMessageReaction` on the operator's own inbound message, so both ends of the hop are visible with zero extra messages (best-effort — a client or Bot API that rejects it never blocks delivery). Stage 2 is exactly one `✅ captain received` text once the message is with the captain, gated off for a `held` outcome (the message is behind an open modal; acking it would arm a watchdog for a turn nobody was given). A new `inbound-lifecycle.ts` owns the typing keep-alive (re-issued every 4s, started when stage 2 completes and stopped when the captain replies); pending state persists in `telegram-state.json`, so a daemon restart resumes a live expectation or clears a stranded one instead of leaving a phantom typing. The same keep-alive tick enforces the 15-minute watchdog, warning once per pending delivery from a real pane read.
- **A live daemon-log tab in the web dashboard (#519, #843).** A new Logs tab tails `squadrantd.log` into a scrolling viewport over the dashboard's existing SSE transport (new `GET /logs` route) via a bounded ring tailer (256KB seed, 1000-line ring, no new deps) that tolerates a missing/truncated log and carries partial lines across polls. The panel adds a severity filter, pause/resume, copy, and an autoscroll-preserving viewport; existing tabs are untouched.
- **A provider-preset chooser for `squadrant init` (#826, #827).** A fresh install no longer assumes an Anthropic credential: one interactive provider question (or `--preset <a|b|c|d>`) selects claude/auto, opencode, claude+router+gate, or codex. It probes `claude auth status` and warns rather than silently defaulting to claude when no credential exists, fills only a wholly-absent `roles`/`permissions` block on an existing config (never clobbers router/gate), sets `crewRouting` so tiers do not route back to claude, and prints exactly what will change before writing.

### Fixed

- **The Telegram bridge no longer 409-storms when a stray second `getUpdates` consumer shares the bot token (#830, #831).** A token-hash lockfile gate (atomic `O_EXCL` in `os.tmpdir`, stale-reclaimed via owner-pid liveness / mtime age) is acquired before the poll loop, so a second consumer refuses to poll and logs once instead of fighting for the slot; an `AbortController` threaded through `getUpdates`/`stop()` cancels an in-flight long-poll cleanly, and repeated 409s are terminal (bounded retries, one log).
- **Telegram receipts now reflect the final delivery verdict, not a transient probe (#834, #769, #837).** A claude session already busy before a send reports `queued` (the quiet happy path) instead of `accepted + unconfirmed`; `createIsCaptainAlive` is now tri-state (`alive | dead | unknown`), so a failed/absent probe is no longer evidence of death and the bridge stays quiet for an `unknown` captain; and a pre-fallback `gone` channel outcome is cleared once the mailbox/pane fallback delivers.
- **Telegram media ingress and unbound topics are no longer silent drops (#768, #591, #321, #844).** Inbound photos/documents/voice/stickers with a caption now forward the caption (or a standalone marker naming the unforwarded attachment) as the captain message, with a matching `<attachment> not forwarded` receipt; a message in a topic the bridge cannot resolve to a project now logs the thread id and replies into the topic with the one-line fix instead of vanishing.
- **opencode captain pane delivery is now agent-appropriate instead of deferring forever with `reason=no-box` (#786, #845).** The claude-tuned `parseDraftFromScreen` returned null for opencode's `┃`/`╹▀▀` box, so a new `parseOpencodeDraftFromScreen` gate returns deliverable `""` only when the opencode box is positively confirmed empty and defers on every other state — it never extracts draft text and never runs the backspace probe, so no keystroke is injected into an unconfirmed pane. The claude path stays byte-for-byte unchanged.
- **The stage-2 ACK is no longer gated by the auto-launch ensure (#848, #849).** `handleProjectTopic` awaited `ensureCaptainAlive()` (up to the 120s warmup) before ACKing, so a live captain with a transiently-missed health probe made the operator wait two minutes; the ensure now runs in the background and every receipt is decided by the delivery verdict, with the probe-derived `❌ couldn't reach` / `📨 delivered` receipts removed.
- **The Telegram inbound poll self-heals after a terminal 409 stop (#850, #851).** A restart's predecessor holding the `getUpdates` slot could trip the attempt-count guard within seconds and silently kill inbound for the daemon's life. The guard now yields only when a different live process holds the poll lock and otherwise tolerates a burst, and a terminal stop schedules a self-healing re-attempt with backoff so inbound recovers without operator action.
...
- **Permission gate for router backends (U7, #772, #782, #819).** Claude Code's built-in auto-classifier is hardcoded to Claude Sonnet 5 and fails **closed** on a router backend, so `Write`/`Bash` were denied and an unattended crew could not work. Ships a `PermissionRequest`-hook gate (`squadrant gate claude permission-request`) that classifies with the configured router model (no Anthropic credential) and steps aside when a real Claude subscription returns. It is a pure decision engine plus a file-backed decision cache: a Tier-1 static deny-rule pass (~0 ms, no model) and a Tier-2 router classifier on ambiguity that **asks** — never silently denies — when the classifier is unreachable. Input is injection-safe (user intent + bare tool payload only), the shipped `rm` deny rule matches recursive+force in any flag order (`-rf`/`-fr`/`-r -f`/`--recursive --force`), and the installer migrates the legacy `hooks` command out so the gate is the single owner of the event. New `defaults.gate {mode,model,policy,tools,cache,deny}` config with `SQUADRANT_GATE`/`_MODEL`/`_POLICY`/`_TOOLS`/`_CACHE` env overrides; side sessions carry `SQUADRANT_SIDE_SESSION=1` so the gate can identify them.
- **Per-project routed model + cost observability (U5, #772, #777, #818).** Surfaces the model and `usage.cost` captured by the router shim tee: `RouterUsage` gains `model` (read from the request body) and a new in-memory `RouterUsageLedger` accumulates per project grouped by model, exposed via `RouterService.usage(project)`. The daemon adds a read-only `router-usage` socket verb and a per-project `routerUsage` field on the snapshot (the status-board/dashboard data source). `squadrant crew tasks` appends a cost footer with a per-model breakdown (queried only when `defaults.router` is configured), and terminal (`task.done`/`task.failed`) Telegram crew events append a one-line cost summary. Granularity is per project; per-crew attribution requires per-crew tokens and is documented as follow-up.

### Changed

- **Router backend documentation + release verification checklist (U6, #772, #778, #817).** Adds a **Router Backend (Harness/Provider Decoupling)** section to `docs/reference.md`: the full `defaults.router` config block matching the U2 schema and its field table, a step-by-step OpenRouter walkthrough (with CCR/LiteLLM/custom as alternatives), the model-id alias layer including the known models-map gap, the explicit backward-compat guarantee and a migration note (opt-in, nothing to do), and a release verification checklist (native unchanged; routed crew end-to-end per the U1 live acceptance).
