...
- **`captainChannel`** (`off` / `shadow` / `on`) — governs whether crew wrapper/receipt text is visible in the captain's own pane. `on` is noisier (you see the receipt plumbing) but delivery-verified; `shadow` keeps captain panes clean while still cross-checking against the old inference path.
