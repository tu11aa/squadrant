...
Confirmed **genuinely generated, not hand-edited** (single non-marked file is the `index.ts` barrel). Sitting inside `src/` it: bloats `@squadrant/agents` (5,384 of its 8,539 "real" LOC), slows every `tsc`/`tsup` pass over the package, and inflates the symbol count — a large share of the 4,625 symbols are inert type aliases.
...
agents/
  ...
  vendor/codex-protocol/   ← S5: generated ts-rs mirror, excluded from build graph
...
| 2 | **S5 — vendor the generated types** | Move `agents/src/codex/protocol/**` → `agents/vendor/codex-protocol/**`; exclude from tsconfig build graph; repoint the barrel import sites. Record regeneration provenance. | **Low** — generated code, single barrel seam | full suite green; build faster; confirm symbol count drops |
...
| 4 | **S4 — naming/ownership cleanup** | Folded into whichever PRs above touch the files: drop the `DeferDelivery` re-export in `cmux.ts` (import from `core` directly); disambiguate `daemon.ts`/`daemon/` and `delivery/`/`daemon/delivery.ts` twins; resolve `workspaces/cmux/` vs `runtimes/cmux.ts`. Use a rename tool, never find-and-replace. | **Low** — rename/re-export only | full suite green; git diff shows only renamed symbols |
