...
anything that touches crews: `squadrant crew` commands, the daemon (`squadrantd`),
...
|-------|-----------|----------|-------------|
| claude | Stop hook → `squadrant crew _hook` → `task.turn.completed` | `src/control/interactive/claude.ts:198` | FLAKY — Stop hook may not fire (#187) |
...
**TYPE 2 — Explicit terminal DONE / BLOCKED / FAILED** (the crew *decides*; the model runs `squadrant crew signal …`):
...
- **claude / opencode:** `squadrant crew signal done|blocked|failed` reads `SQUADRANT_CREW_TASK_ID` from the shell env (set on their cmux shell launch).
- **codex:** the command needs `--task-id <id> --project <project>` (codex shares ONE app-server across threads; `startThread` has no env param, so no per-thread `SQUADRANT_CREW_TASK_ID`). The codex crew template already injects these flags into its `developerInstructions` (PR #173). When driving a codex crew manually, pass `--task-id` / `--project` explicitly — do NOT use the bare claude-style command (it errors `SQUADRANT_CREW_TASK_ID unset`).
...
- **Action:** `squadrant crew spawn &lt;project&gt; "<task>" --name lifecycle-test`
- **Expect:** crew boots a *live* session (not print-mode), accepts the first turn, does work,
  and replies. You can send follow-ups with `squadrant crew send` and get answers.
- **Captain signal:** crew tab is live; `squadrant crew read &lt;project&gt; lifecycle-test` shows its reply.
- [ ] PASS

### 2. Question → captain answers
- **Action:** Tell the crew to ask the captain a genuine clarifying question via
  `squadrant crew signal blocked --question "<q>"`, then wait.
- **Expect:** crew blocks awaiting the answer.
- **Captain signal:** **CREW BLOCKED** (with the question) reaches the captain.
- **Resolve:** `squadrant crew send &lt;project&gt; lifecycle-test "<answer>"` — crew unblocks and proceeds
  using the answer.
- [ ] PASS — blocked notification received **and** answer unblocked it
> **Codex note:** Signals via the crew's own `developerInstructions` (PR #173) which inject `--task-id <id> --project &lt;project&gt;`. When driving codex manually, pass those flags explicitly — the bare `squadrant crew signal...` errors `SQUADRANT_CREW_TASK_ID unset`. See §"Two Signal Mechanisms" above.
...
- **Resolve:** `squadrant crew send &lt;project&gt; lifecycle-test "1"` (or the approve option) — the
...
- **Captain signals to verify:**
  - Real turn-end → **CREW IDLE** in the captain mailbox; `squadrant crew list` shows `awaiting-input`.
...
- **Action:** Tell the crew to run `squadrant crew signal done --message "<one-line summary>"`.
  > **Codex note:** same `--task-id <id> --project &lt;project&gt;` requirement as CP2; prefer the crew's self-signal via `developerInstructions`.
...
- **Action:** With the crew already terminal `done` (checkpoint 5), the captain reviews, finds a gap, and sends a new turn with redo instructions: `squadrant crew send &lt;project&gt; <name> "<redo>"`.
- **Expect:** `squadrant crew send` detects the terminal task and fires `task.reopened` (#148) — the task returns to non-terminal `working`. The crew does the redo, then runs `squadrant crew signal done` again.
...
- [ ] `squadrant crew close &lt;project&gt; lifecycle-test`
...
- **Root cause (confirmed live).** Codex crews dispatched under `sandbox: "workspace-write"`. The daemon socket (`~/.config/squadrant/squadrant.sock`), cmux socket, and LaunchAgents plist all live outside the workspace, so Seatbelt denied the `connect()`/writes — `squadrant crew signal` could never reach the daemon (`touch ~/.config/squadrant/… → Operation not permitted`). Only app-server paths (CP1, CP3) worked; the explicit-signal lifecycle (CP2/5/6) was dead.
